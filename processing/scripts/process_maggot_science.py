...
for graph_type in graph_types:
    ...
    edgelist[f"{graph_type}_weight"] = edgelist[f"{graph_type}_weight"].astype("Int64")
    edgelist.set_index(["source", "target"], inplace=True)
    edgelists.append(edgelist)
# %%
edgelist = pd.concat(edgelists, axis=1)
...
edgelist.to_csv(OUT_PATH / "maggot_brain" / "edgelist.csv")
edgelist.to_csv(OUT_PATH / "maggot_brain" / "edgelist.csv.gz")
...
nodes.rename(columns={"celltype": "super_class"}, inplace=True)
...
# %%
nodes.to_csv(OUT_PATH / "maggot_brain" / "nodes.csv")
nodes.to_csv(OUT_PATH / "maggot_brain" / "nodes.csv.gz")
